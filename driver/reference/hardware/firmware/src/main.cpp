...
#define DATA_PACKET_SEND_INTERVAL 50
    ...
    while (1) {
        ...
        if (xSemaphoreTake(params->magnetometer_mutex, portMAX_DELAY) ==
            ...
            params->magnetometer.x =
                magnetometer_read_sum_x_y_z[0] / READ_COUNT_FOR_AVERAGE;
            params->magnetometer.y =
                magnetometer_read_sum_x_y_z[1] / READ_COUNT_FOR_AVERAGE;
            params->magnetometer.z =
                magnetometer_read_sum_x_y_z[2] / READ_COUNT_FOR_AVERAGE;
...
void send_packet(void* pvParameters) {
    ...
    while (1) {
        ...
        if (xSemaphoreTake(params->gnss_location_mutex, portMAX_DELAY) ==
            pdTRUE) {
            packet.timestamp = params->gnss_time.is_valid
                                   ? gnss_get_timestamp(&(params->gnss_time))
                                   : 0;
...
void setup() {
    ...
    if (params.magnetometer_mutex == NULL ||
