...
func (p *PanTilt) FindNorth(port io.ReadWriteCloser, driver pan_tilt.PanTiltDriver, state *features.State) (float64, error) {
	const (
		FAST_STEP     = 10
		SLOW_STEP     = 0.2
		ERR_THRESHOLD = 0.1
	...
	for i := 0; i < 360/FAST_STEP; i++ {
		...
		azimuthData = append(azimuthData, state.Compass.MagAzimuth)
	}
	...
	for {
		...
		currentAzi := state.Compass.MagAzimuth
		...
		p.OnEvent(nil, fmt.Sprintf("current compass azimuth: %.2f", currentAzi))
	}
